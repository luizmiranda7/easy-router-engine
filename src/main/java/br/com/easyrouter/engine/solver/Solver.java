...
public class Solver {
	 ...
	 */
	private Solver(RouteRequest routeRequest) {
	...
	private void loadVehicles(Set<Vehicle> vehicles, DistributionCenter distributionCenter){
		for (Vehicle vehicle : vehicles) {
			VehicleType vehicleType = VehicleTypeImpl.Builder
					.newInstance(vehicle.getExternalCode().toString())
					.addCapacityDimension(WEIGHT_INDEX, vehicle.getTotalWeight().intValue())
					.addCapacityDimension(VOLUME_INDEX, vehicle.getTotalVolume().intValue())
					.setCostPerDistance(vehicle.getCostPerDistance())
					.setCostPerTime(vehicle.getCostPerTime())
					.setMaxVelocity(vehicle.getMaxVelocity())
					.build();
			
			this.vehicleImpls.add(VehicleImpl.Builder
					.newInstance(vehicle.getExternalCode().toString())
					...
					.setLatestArrival(vehicle.getLatestEnd().getTime())
					.setType(vehicleType)
			.build());
		}
	}
	...
	private void loadOrders(Set<Order> orders){
		...
		for (Order order : orders) {
			...
			services.add(Service.Builder
				.newInstance(order.getExternalCode().toString())
	 ...
	 */
	public void solve(final RouteRequest routeRequest) {
	}
}
